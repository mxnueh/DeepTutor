...
初めてのローカル Web セットアップのためのガイド付き CLI ウィザード。環境チェック、Python と Node.js 依存関係のインストール、`data/user/settings` の JSON 設定作成、TutorBot・Matrix・Math Animator などのオプションアドオン選択ができます。
...
`data/user/settings/*.json` を編集して少なくとも LLM フィールドを入力してください。埋め込みフィールドはナレッジベース機能に必要ですが、チャットを試すだけであれば後回しにできます。
...
`data/user/settings/*.json` を編集して少なくとも必須フィールドを入力してください（[オプション B](#option-b--manual-local-install) と同様）。
...
```bash
python scripts/docker_compose.py logs -f   # ログのフォロー
python scripts/docker_compose.py down       # コンテナの停止と削除
```

<details>
<summary><b>クラウド / リモートサーバーデプロイ</b></summary>

リモートサーバーにデプロイする場合、ブラウザはバックエンド API の公開 URL を知る必要があります。`data/user/settings/*.json` にもう1つ変数を追加してください：
...
PocketBase は組み込みの SQLite/JSON 認証とセッションストレージを置き換えるオプションの軽量バックエンドです。OAuth 対応認証、リアルタイムサブスクリプション、ビジュアル管理パネルを追加します — `integrations.pocketbase_url` を設定しなければいつでも元に戻せます。

> ⚠️ **PocketBase モードは現在シングルユーザーのみ対応。** デフォルトスキーマには `users` に `role` フィールドがなく（すべてのログインが `role=user` になり、管理者を作成できない）、セッション/メッセージ/ターンのクエリが `user_id` でフィルタリングされていません。マルチユーザーデプロイは `integrations.pocketbase_url` を空白のままにしてデフォルトの JSON/SQLite バックエンドを使用してください。
...
# data/user/settings/*.json で PocketBase を有効化して再起動
```

**必須 `data/user/settings/*.json` 追加：**
...
`integrations.pocketbase_url` を未設定（または削除）にすれば、いつでも組み込みバックエンドに戻れます — 新しいセッションのデータ移行は不要です。
...
`data/user/settings/*.json` でデフォルトポートをオーバーライド：
...
```bash
python scripts/docker_compose.py up -d     # または python scripts/docker_compose.py -f docker-compose.ghcr.yml up -d
```
...
これらのディレクトリは `python scripts/docker_compose.py down` 後も残り、次の `python scripts/docker_compose.py up` で再利用されます。
...
```bash
python scripts/start_tour.py   # data/user/settings/*.json を編集して API キーを入力
```
...
> ⚠️ **PocketBase モード（`integrations.pocketbase_url` 設定済み）はシングルユーザーのみ対応。** デフォルトの PocketBase スキーマには `users` に `role` フィールドがなく（すべてのログインが `role=user` になり、管理者を作成できない）、`sessions` / `messages` / `turns` クエリが `user_id` でフィルタリングされていません。マルチユーザーデプロイは `integrations.pocketbase_url` を空白のままにしてデフォルトの JSON/SQLite バックエンドを使用してください。

> ⚠️ **シングルプロセス推奨。** 最初のユーザーの管理者昇格はプロセス内 `threading.Lock` で保護されています。マルチワーカーデプロイは最初の管理者をオフラインでプロビジョニングする（`auth.enabled=false` で起動してブートストラップフローを完了してからフラグを切り替え）か、外部システムでユーザーストアをバックアップしてください。
